...
function Links({ Icon, click, title }: { Icon: React.ComponentType, click: string, title: string }) {
    return (
        <div>
            <Link className="border border-neutral-800 rounded-full w-10 h-10 flex justify-center items-center hover:cursor-pointer hover:scale-125 transition-all ease-in-out duration-500" href={click} title={title}>< Icon /></Link>
        </div>
    )
}